...
class ControllerExtensionMkenqprolist extends Controller {
	...
	public function sendenquirymail() {
		if(isset($this->request->post['mkenqpro_id']) && $this->request->post['mkenqpro_id']) {
			...
			$mkenqpro_data = $this->model_extension_mkenqprolist->getmkenqpro($this->request->post['mkenqpro_id']);
	...
	public function upload() {
		...
		if(isset($_FILES["attachmentfile"])) {
			$attach = true;
			$target_attachment_file = DIR_IMAGE . '/sendquotemail_upload/' . basename($_FILES["attachmentfile"]["name"]);
	...
	public function sendmail() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!$this->request->post['fromemail']) {
				$json['error']['fromemail'] = $this->language->get('error_fromemail');
			}
			...
			if (!$json) {
				...
				$Fromemails = $this->request->post['fromemail'];

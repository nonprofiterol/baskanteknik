  ...
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right">
        <button id="button-send" data-loading-text="<?php echo $text_loading; ?>" data-toggle="tooltip" title="<?php echo $button_send; ?>" class="btn btn-primary" onclick="send();"><i class="fa fa-envelope"></i> <?php echo $btn_sendmail; ?></button> </div>
      ...
      <div class="panel-body">
		...
		<input type="hidden" name="mkenqpro_id" value="<?php echo $mkenqpro_data['mkenqpro_id'];?>" />
		  ...
		  <div class="form-group required">
            <label class="col-sm-2 control-label" for="input-fromemail"><?php echo $entry_from; ?></label>
            <div class="col-sm-10">
				<input type="text" name="fromemail" id="input-fromemail" class="form-control" />
            </div>
          </div>
		  <div class="form-group required">
 	...
 	$.ajax({
		...
		success: function(json) {
			...
			if (json['error']) {
				...
				if (json['error']['fromemail']) {
					$('input[name=\'fromemail\']').after('<div class="text-danger">' + json['error']['fromemail'] + '</div>');
				}
			...
			if (json['success']) {
				$('#sendenquirymail_model .modal-body').html('<div class="alert alert-success"><i class="fa fa-check-circle"></i>  ' + json['success'] + '</div>');
				
				$('#sendenquirymail-form')[0].reset();
			}

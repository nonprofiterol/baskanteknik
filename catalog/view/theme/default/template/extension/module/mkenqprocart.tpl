    ...
    <div id="content" class="<?php echo $class; ?>"><?php echo $content_top; ?>
      ...
      <div class="table-responsive">
        <form method="post" name="submitmkenqpro" id="submitmkenqpro" action="index.php?route=<?php echo $modpathcart;?>/submitmkenqpro">
          <div class="form-group col-sm-12 groupdiv">
		  ...
		  <?php } else { ?>
            ...
            <div class="col-sm-10">
				<span class="text-danger" id="error-input-captcha"></span>
          ...
          <div class="form-group col-sm-12 groupdiv">
            <button type="button" class="btn btn-primary" id="mkenqprosubmit" onclick="submitdatamkenqprod();"><?php echo $btn_submit;?></button>
 			<a href="<?php echo $continue_btn_href; ?> " class="btn btn-default" data-dismiss="modal"><?php echo $btn_continue;?></a>
          </div>
        </form>
      </div>
...
function updatecart_mkenqprocart() {
 	$.ajax({
 		url: 'index.php?route=<?php echo $modpathcart;?>',
 		dataType: 'html',
		beforeSend: function() {
			$('#content').html('<img src="image/loader.gif"/>'); 
		}, 
...
function submitdatamkenqprod() {   
	...
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#content').html(json['response']); 
		}
	}); 
}
</script>
<style type="text/css">.groupdiv{padding:10px;clear:both}</style>
<?php echo $footer; ?> 